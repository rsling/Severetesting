...
sev2 <- 1-qnorm(p = 0.4, mean = 0.3, sd = se, lower.tail = T)

plot(dnorm(xs, mean = 0, sd = se), type = "l", xaxt = "n", col = 1, lwd = 2,
     main = "Severity for observed mu[obs] = 0.4 ; H0: mu = 0",
...
lines(dnorm(xs, mean = 0.3, sd = se), col = "orange", lwd = 2)
lines(rep(which.min(abs(xs - 0.3)), 2), c(0, dnorm(0.3, mean = 0.3, sd = se)), col = "orange", lwd = 1, lty = 3)
...
y3.all <- dnorm(seq(0.4, 1.5, 0.01), mean = 0.3, sd = se)
polygon(c(x.start, x.start:x.end), c(0, y3.all), col = "violet")
...
text(x = 35, y = 1.2, labels = paste0("Tail for rejection of H0\np = ", round(ps[1], 2)), col = "darkblue")
...
text(x = 200, y = 1.2, labels = paste0("SEV tail for H1': mu = 0.2\nif mu[obs] = 0.4\nSEV = 1 - ", round(1-sev, 2), " = ", round(sev,2)), col ="darkgreen")
text(x = 200, y = 0.8, labels = paste0("SEV tail for H1'': mu = 0.3\nif mu[obs] = 0.4\nSEV = 1 - ", round(1-sev2, 2), " = ", round(sev2,2)), col ="violet")
